...
class QueueManager:
    ...
    def ordered_jobs(self):
        ...
        return (
            Job.objects.filter(status__in=[
                               Job.JobStatus.PENDING.value, Job.JobStatus.INTERRUPTED.value, Job.JobStatus.CALC_ERROR.value])
    ...
    def connection_lost_jobs(self):
        ...
        return (
            Job.objects.filter(
                status__in=[Job.JobStatus.CONNECTION_LOST.value])
        )
    ...
    def prepare_job(self):
        ...
        if job:
            ...
            # Exit the for loop to start over with new tasks in the queue
                ...
                log.warning(
                    "No available GPUs. Waiting 30 seconds to retry...")
                time.sleep(30)
